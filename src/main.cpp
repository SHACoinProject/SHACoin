...
// Copyright (c) 2014  The SHACoin developer
...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 365 * 300; //unlimited
unsigned int nStakeTargetSpacing = 1 * 60; // 60 seconds block spacing
const int64 nChainStartTime = 1394985600; // 2014-3-16 16:00:00 GMT
const int64 nTestNetStartTime = nChainStartTime; 
int nCoinbaseMaturity = 480; // minted blocks need need 500 confirm
...
static const int64 nTargetTimespan = 0.16 * 24 * 60 * 60;  
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal SHACoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SHACoin
    bool fStrictPayToScriptHash = true; // Always active in SHACoin
    ...
    // ppcoin: fees are not collected by miners as in SHACoin
    ...
    if(pindex->pprev)
    {
        ...
        if (vtx[0].GetValueOut() > GetProofOfWorkReward(pindex->nHeight, nFees, prevHash) && pindex->nHeight != MIN_SUPPLY)
            return false;
    }
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SHACoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Missing plane sent signals to satellite for hours";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:SHA genesis block";
...
unsigned char pchMessageStart[4] = { 0xe2, 0xe4, 0xe5, 0xe6 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("SHACoinMiner : proof-of-work not meeting target");
...
void static ThreadSHACoinMiner(void* parg);

static bool fGenerateSHACoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void SHACoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("SHACoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateSHACoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateSHACoins) && !fProofOfStake)
                return;
        }
...
void static ThreadSHACoinMiner(void* parg)
{
    ...
    try
    {
        ...
        SHACoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadSHACoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadSHACoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadSHACoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateSHACoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateSHACoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateSHACoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d SHACoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadSHACoinMiner, pwallet))
                printf("Error: NewThread(ThreadSHACoinMiner) failed\n");
            Sleep(10);
        }
    }
}
